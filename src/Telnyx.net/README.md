...
The [`TelnyxWebhook`](https://github.com/thedonmon/telnyx-dotnet/blob/master/src/Telnyx.net/Entities/TelnyxWebhook.cs) object that abstracts away the POCO format the webhook is received as.

**Example: Access a TelynxWebhook**
```csharp
[ApiController]
[Route("messaging/[controller]")]
public MyIncomingMessagesController : ControllerBase
{
    [HttpPost]
    public async Task<IActionResult> MessageCallback([FromBody]TelnyxWebhook<MessageWebhook> req)
    {
        //.. do stuff
        return Ok();
	}
...
*NOTE*: If you are using a .NETCORE 3.0+ API application, the new default is to use `System.Text.Json` as the JSON serializer/deserializer. 
Please add the following line(s) to your application in order to avoid any issues during webhook receipt.

Install the extension package
`Install-Package Microsoft.AspNetCore.Mvc.NewtonsoftJson`

Depending on your project append the `.AddNewtonsoftJson()` to your services like so:
```
services.AddControllers().AddNewtonsoftJson(); //May only need this line if it's just an API application
services.AddControllersWithViews().AddNewtonsoftJson(); //If using views
services.AddRazorPages().AddNewtonsoftJson(); //If using RazorPages

```

The SDK will be updated for .netstandard2.0+ to support `System.Text.Json` soon. 

```
**Webhook Type Definition**

The safest way to consume a webhook is to pass a TelnyxEntity as the <T> parameter, however if for whatever reason the object is not defined in the SDK
or you prefer to use your own POCO to consume the data points you are interested, you are free to do so. 
